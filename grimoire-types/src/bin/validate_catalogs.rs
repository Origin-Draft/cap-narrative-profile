    ...
    // shape via typed adapter (canonicalized to RelationshipRolesCatalogDocument).
    {
        ...
        if full.exists() {
            match fs::read_to_string(&full) {
                Err(e) => errors.push(format!("{}: {}", path, e)),
                Ok(content) => match grimoire_types::catalogs::parse_relationship_roles_catalog(&content) {
                    Ok((doc, used_legacy)) => {
                        if used_legacy {
                            println!(
...
fn dump_schemas(root: &Path) {
    let schemas = grimoire_types::generate_all_schemas();
